...
| Meta Google-Sheet import (Phase 4a) | **DEV VERIFIED → DEPLOYED (401 fix live 2026-09-03, commit 1e386a0; Render cron activation pending)** | 392 leads imported earlier via dev admin UI; the middleware 401 root cause was fixed and deployed; the remaining gap is Render-side only — the two cron services don't exist yet (see Deployment Checklist) |
...
| Phase 3 — Website leads (enquiry form + abandoned bookings) | **BUILT 2026-09-02 — migration 099 applied; dev verification pending** | Contact form + telemetry + sweep built; capture-speed upgrades 2026-09-02 (user-attached sessions, 10-min staleness, booked auto-resolution); sweep scheduling now via Supabase pg_cron every minute (migration 101, apply pending) |
...
| Cron automation + scheduling | **REDESIGNED 2026-09-03 (owner decision): Supabase pg_cron + pg_net replaces Render cron services — route-side heartbeats DEV VERIFIED; migration 101 + sheet push trigger BUILT; owner apply/install pending** | Sweep every minute + import every 5 min (fallback for the Apps Script push trigger that imports within seconds of a new sheet row) — $0 extra vs ~$6/mo for two Render crons, hot leads ~10–11 min after quiet vs 10–15; render.yaml CRM cron declarations removed; see Deployment Checklist + Decision Log |
| Automation observability (heartbeats + health panel) | **DEPLOYED (1e386a0) + ROUTE-SIDE HEARTBEATS DEV VERIFIED 2026-09-03** | Public heartbeat endpoint + panel + Discord alerts live in prod; the import + sweep route handlers now record heartbeats themselves for secret-auth runs (12 unit tests + localhost E2E vs prod DB — first-ever sweep heartbeat landed), so every trigger path (pg_cron, sheet push trigger, runner scripts) is tracked; flips to Healthy once migration 101 is applied |
...
- [ ] Automation activation (replaces the original Render-cron plan): apply migration 101 (pg_cron schedules + secret in `private.crm_automation_config`) + install the Apps Script push trigger — **required before production automation**; the web-service env + secret were already confirmed set 2026-09-03
...
- [ ] **Commit this change set + deploy to Render** — the route-side heartbeat code must be live BEFORE (or together with) migration 101: until it deploys, pg_cron-triggered runs would still import/sweep correctly but their heartbeats would not record, so the panel would wrongly show stale while runs succeed
- [ ] **Apply migration `infra/supabase/migrations/101_crm_automation_pg_cron.sql`** — BUILT: enables pg_net, creates `private.crm_automation_config` (secret store, non-public schema + revoked from anon/authenticated) + `private.crm_trigger_automation()` (SECURITY DEFINER, pg_net POST with the Bearer secret) + schedules `crm-abandoned-bookings-sweep` (every minute) and `crm-meta-sheet-import` (`*/5 * * * *`). Owner steps: replace the `<PASTE_THE_CRM_SHEET_IMPORT_SECRET_HERE>` placeholder with the **web service's** secret value BEFORE applying, then run in the Supabase SQL editor (pg_cron is already enabled on this project — migrations 063/066/073; this migration fails loudly if extensions are missing, unlike 073's silent skip)
- [ ] **Install the Google-Sheet push trigger** (`infra/apps-script/crm-sheet-push-trigger.gs`) on the leads spreadsheet — paste the script, set `ENDPOINT_SECRET` (same secret), Run → testNow (approve the permission; expect 200 + a fresh import heartbeat on the panel), then Triggers → Add Trigger → onSheetChange → From spreadsheet → On change. New leads then import within seconds of landing in the sheet; the 5-min pg_cron import is the fallback safety net
- [ ] **Verify activation**: `select jobname, schedule, active from cron.job;` shows both jobs active; the CRM "Automation health" panel shows both jobs **Healthy** (sweep heartbeats every ~1 min, import every ~5 min); a real `trigger_source='cron'` / `dry_run=false` import run appears in `crm_sheet_import_runs` with lead KPIs holding
- [ ] Real cron import + lead KPI verification — happens automatically once migration 101 is applied (the dry-run half is DONE 2026-09-03 via the localhost E2E: real sheet scanned, 405 rows / 401 candidates / 0 imported as expected for a dry run, heartbeats landed)
