...
| SOP checklist (order completion gate + photo evidence) | API WIRED (WEB) / BACKEND CONTRACT READY | Provider app file-picker upload, completion-gate device E2E; requires migration 103 applied |
...
| 2026-09-07 | Platform implementation agent | Provider SOP checklist backend contract (web-first rollout) | `npm test && npm run lint && npx tsc --noEmit` (focused: `lib/bookings/sop-assignments.test.ts`, `app/api/bookings/[id]/status/__tests__/route.test.ts`) | PASSED | Added migration 103 (`provider_sops`, `booking_sop_submissions`, `booking_sop_photos`, booking waiver columns, `sop-photos` bucket, 3 seeded SOPs), provider completion gate in `applyBookingStatusTransition`, provider APIs `GET /api/provider/bookings/[id]/sops` + `POST .../sops/[submissionId]/photos` (bearer-compatible), `sop-photos` signed-upload support, admin SOP CRUD + review/waive/bypass routes, and web provider/admin SOP UI. Full suite: 477 passed / 1 skipped; lint clean; non-test tsc clean (pre-existing test-file type errors unchanged). Mobile app UI for SOP photo upload remains NOT STARTED — `dofurs-mobile/` workspace is not present in this repository, so no mobile code was changed or device-verified in this session. |
| 2026-09-07 | Platform implementation agent | Migration 103 applied to the environment DB (surgical, single-file) | `psql -v ON_ERROR_STOP=1 -f infra/supabase/migrations/103_provider_sop_checklists.sql` (psql via Homebrew `libpq`, direct 5432 connection from `.env.local` `SUPABASE_DB_URL`) | PASSED | Applied only 103 (migrations 060–102 were confirmed already present in the database; `schema_migrations` bookkeeping was empty so the bulk runner was intentionally not used). Atomic `begin/commit` run: only expected `drop-if-exists` notices, clean COMMIT. Post-verify (read-only): `bookings` row count unchanged (136 before/after — zero data impact), 3 SOP tables + 4 waiver columns + `sop-photos` bucket created, 3 seed SOPs active/mandatory/photo-required, RLS on all new tables (9 table policies + 2 storage policies), bookkeeping row for 103 recorded. |
| 2026-09-07 | Platform implementation agent | Supabase types regenerated from the live schema | `SUPABASE_PROJECT_ID=eyrmedtujdfeohdoofrx npm run db:types` then `npx tsc --noEmit && npm test && npm run test:schema-health` | PASSED | `lib/supabase/database.types.ts` regenerated from the environment DB (token from `.env.local`); now includes SOP tables/columns (FK names match the prior manual entries) plus earlier 060–102 tables the committed file was missing. Post-regen: tsc unchanged (only 3 pre-existing test-file errors), full suite 477 passed / 1 skipped, schema-health smoke 23/23 PASS. Graphify rebuilt after the types change. |
| 2026-09-07 | Platform implementation agent | `schema_migrations` bookkeeping backfilled for 060–102 (pure bookkeeping, zero schema/data change) | Sentinel verification via read-only psql checks (43/43 PASS: table/column/function/policy/trigger/index/FK/constraint/timezone/campaign-row sentinels per file) then single `INSERT ... ON CONFLICT (filename) DO NOTHING` transaction | PASSED | All 43 migrations 060–102 verified already present in the live DB before recording (33 via auto-detected sentinels, 10 via targeted checks incl. IST timezone setting, `bookings_provider_service_id_fkey` ON DELETE SET NULL, `private.crm_automation_config`, `welcome_offer` campaign row). Note: 082's row exists but was legitimately changed by operations to `PETFIRST100`/₹100 (data drift, not missing migration) — 082 must NOT be re-run as it would overwrite that decision. Post-backfill: `node scripts/supabase-migrations.mjs status` reports Total: 44 | Pending: 0; schema-health smoke PASS. |
...
| 2026-09-07 | Ship provider SOP completion gate web-first; provider mobile app consumes the same bearer-compatible APIs (`GET /api/provider/bookings/[id]/sops`, `POST .../sops/[submissionId]/photos`, extended `/api/storage/signed-upload-url` with `sop-photos`) when its upload work resumes. | Enforcement must not wait on mobile readiness; server-side gate in `applyBookingStatusTransition` applies to every provider surface (web and future mobile) while admin/staff transitions stay exempt with recorded waivers. | Platform implementation agent |
