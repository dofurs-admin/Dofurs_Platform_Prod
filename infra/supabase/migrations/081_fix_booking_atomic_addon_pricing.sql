...
-- ============================================================================
-- 081_fix_booking_atomic_addon_pricing.sql
-- Purpose: Update create_booking_atomic to price add-ons from the normalised
--          addon catalog tables (provider_service_addon_mappings + addon_templates)
--          introduced in migration 080. The previous implementation joined the
--          legacy service_addons table, so the server-side add-on price was
--          always 0 when new-system mappings were used.
-- ============================================================================
...
begin
  ...
  begin
    ...
    -- SECURITY: Discount calculation MUST happen server-side only
    ...
    -- SECURITY: Add-on total calculated server-side only.
    -- IDs in p_add_ons refer to provider_service_addon_mappings.id (new system).
    -- Falls back to service_addons.id for any legacy ID not found in the new table.
    if jsonb_typeof(p_add_ons) = 'array' then
      select coalesce(
        sum(
          coalesce(psam.price_override, tmpl.default_price, 0)
          * greatest((addon.item->>'quantity')::integer, 1)
        ), 0
      )
        into v_add_on_total
      from jsonb_array_elements(p_add_ons) as addon(item)
      join public.provider_service_addon_mappings psam
        on psam.id = (addon.item->>'id')::uuid
       and psam.is_active = true
       and psam.moderation_status = 'approved'
      join public.addon_templates tmpl
        on tmpl.id = psam.addon_template_id
       and tmpl.is_active = true
       and tmpl.moderation_status = 'approved';

      -- Legacy fallback: add any IDs that exist only in service_addons (old system).
      -- This handles bookings created during the transition period.
      v_add_on_total := coalesce(v_add_on_total, 0) + coalesce(
        (
          select sum(coalesce(sa.price, 0) * greatest((addon.item->>'quantity')::integer, 1))
          from jsonb_array_elements(p_add_ons) as addon(item)
          ...
          where not exists (
            select 1
            from public.provider_service_addon_mappings psam2
            where psam2.id = (addon.item->>'id')::uuid
          )
        ), 0
      );
    end if;

    -- SECURITY: Final price ALWAYS calculated server-side - NO CLIENT OVERRIDE
    v_final_price := greatest(v_service.base_price + v_add_on_total - v_discount_amount, 0);
