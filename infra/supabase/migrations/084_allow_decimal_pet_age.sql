...
alter table public.pets
  alter column age type numeric(5,2) using age::numeric(5,2);

alter table public.pets
  drop constraint if exists pets_age_check;

alter table public.pets
  add constraint pets_age_check check (age is null or (age >= 0 and age <= 99)) not valid;
...
declare
  ...
  user_addresses_phone_column_exists boolean;
  user_addresses_phone_constraint_exists boolean;
  ...
  pets_age_is_decimal boolean;
  pets_age_constraint_exists boolean;
  checks jsonb;
  all_ok boolean;
begin
  ...
  select exists(
    ...
    from information_schema.columns c
    where c.table_schema = 'public'
      and c.table_name = 'user_addresses'
      and c.column_name = 'phone'
  ) into user_addresses_phone_column_exists;
  ...
  select exists(
    ...
    join pg_namespace n on n.oid = tbl.relnamespace
    where n.nspname = 'public'
      and tbl.relname = 'user_addresses'
      and c.conname = 'user_addresses_phone_format'
      and c.contype = 'c'
  ) into user_addresses_phone_constraint_exists;
  ...
  select exists(
    ...
    join pg_namespace n on n.oid = tbl.relnamespace
    where n.nspname = 'public'
      and tbl.relname = 'pets'
      and c.conname = 'pets_age_check'
      and c.contype = 'c'
        and pg_get_constraintdef(c.oid) ilike '%age >=%'
        and pg_get_constraintdef(c.oid) ilike '%age <=%'
  ) into pets_age_constraint_exists;
  ...
  checks := jsonb_build_array(
    ...
    jsonb_build_object('key', 'user_addresses.phone.column.exists', 'ok', coalesce(user_addresses_phone_column_exists, false), 'expected', true, 'actual', coalesce(user_addresses_phone_column_exists, false)),
    jsonb_build_object('key', 'user_addresses.phone.constraint.exists', 'ok', coalesce(user_addresses_phone_constraint_exists, false), 'expected', true, 'actual', coalesce(user_addresses_phone_constraint_exists, false)),
    ...
    jsonb_build_object('key', 'pets.age.decimal_numeric', 'ok', coalesce(pets_age_is_decimal, false), 'expected', 'numeric(5,2)', 'actual', coalesce(pets_age_is_decimal, false)),
    jsonb_build_object('key', 'pets.age.constraint.exists', 'ok', coalesce(pets_age_constraint_exists, false), 'expected', true, 'actual', coalesce(pets_age_constraint_exists, false))
  );
