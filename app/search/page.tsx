...
import { BookOpen, HelpCircle, FileText, Bath, Search } from 'lucide-react';
...
export const metadata: Metadata = {
  title: 'Search',
  description: 'Search for grooming packages, grooming guides, FAQs, and information on Dofurs.',
  robots: {
    index: false,
    follow: true,
    googleBot: {
...
const staticPages = [
  ...
  { title: 'FAQs', description: 'Answers to the most common questions about grooming bookings.', href: '/faqs' },
  ...
  { title: 'Become a Grooming Partner', description: 'Join the Dofurs network as a verified pet groomer.', href: links.provider },
];
...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const comingSoonCtaClass = 'inline-flex h-9 items-center justify-center rounded-full border border-amber-200 bg-amber-50 px-4 text-xs font-semibold text-amber-700 opacity-80';
          ...
          <div className="pt-8">
            {hasQuery ? (
              <>
                ...
                {totalResults === 0 && (
                  <p className="mt-3 text-base text-[#675245]">
                    Try a different keyword — for example <span className="font-semibold text-[#3a2c22]">grooming</span>, <span className="font-semibold text-[#3a2c22]">bath</span>, or <span className="font-semibold text-[#3a2c22]">haircut</span>.
                  </p>
                )}
              </>
            ) : (
              <>
                ...
                <p className="mt-3 text-base text-[#675245]">Search for grooming packages, articles, FAQs, and more across Dofurs.</p>
              </>
            )}
          </div>
                    ...
                    >
                      ...
                      <div className="mt-4">
                        {service.availability === 'active' ? (
                          ...
                          <button type="button" disabled className={comingSoonCtaClass}>
                            Coming Soon
                          </button>
                        )}
                      </div>
