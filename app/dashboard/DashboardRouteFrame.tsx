...
export default function DashboardRouteFrame({ children, fontClassName }: DashboardRouteFrameProps) {
  const pathname = usePathname();
  const isAdminRoute = pathname === '/dashboard/admin' || pathname?.startsWith('/dashboard/admin/');

  if (isAdminRoute) {
    return (
      <main className={cn(fontClassName, 'min-h-screen bg-neutral-50 text-neutral-950')}>
        {children}
      </main>
    );
  }
  ...
  return (
    <>
      ...
      <main
        className={cn(
          fontClassName,
          'dofurs-mobile-main relative overflow-hidden bg-[linear-gradient(180deg,#fffcf8_0%,#fffaf6_40%,#fffcf9_100%)] pt-16 text-ink sm:pt-20',
        )}
      >
        <div
